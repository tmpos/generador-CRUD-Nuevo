...
<!--         <div class="panel flex items-center overflow-x-auto whitespace-nowrap p-3 text-primary">
            ...
            <span class="ltr:mr-3 rtl:ml-3">{{titulo}}</span>
        </div> -->
...
const titulo = ref(document.title);

/*const observer = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    if (mutation.type === 'childList' && mutation.target.nodeName === 'TITLE') {
      titulo.value = document.title.toUpperCase();
    }
  });
});*/
...
await nextTick();
  // Configurar el observer para observar cambios en el título
  ...
  // Iniciar la observación del título
