...
export async function imprimirTicket(factura, datosEmpresa) {
  ...
  const datosFactura = await peticionesFetch(`${link}${api}`, `datoscampo/sales/invoice_number/${factura}`, {}, tokenCifrado, 'GET');
  ...
  const datosOtro = datosFactura.other ? JSON.parse(datosFactura.other) : {};
  ...
  const qrCodeData = await QRCode.toDataURL(`${link}/receipt/ticket?factura=${datosFactura.invoice_number}`);
...
<div id="mid" class="bordeado">
    <div class="info">
        <div class="left-column">
            <p>
                ${configuracionfactura.no_factura ? `DOC: <b style="font-size:16px">#${datosFactura.invoice_number}</b><br>` : ''}
                ${configuracionfactura.comprobante ? `NCF: ${datosFactura.receipt}<br>` : ''}
                ...
                ${configuracionfactura.metodopago ? `METODO DE PAGO: ${datosFactura.payment_method}` : ''}
            </p>
        </div>
        <div class="right-column">
            ...
            <div id="subtotal" style="font-weight: bold;">
                <table>
                    <tr>
                        <td>SUBTOTAL:</td>
                        <td style="text-align:right;"><span style="font-size: 1.5em !important;margin-top: 5px;margin-bottom: 5px;">${datosConfiguracion.simbolo}${datosFactura.subtotal}</span></td>
                    </tr>
                </table>
            </div>
            ...
            <div id="impuesto" style="font-weight: bold;">
                <table>
                    <tr>
                        ...
                        <td style="text-align:right;"><span style="font-size: 1.5em !important;margin-top: 5px;margin-bottom: 5px;">${datosConfiguracion.simbolo}${datosFactura.tax}</span></td>
                    </tr>
                </table>
            </div>
